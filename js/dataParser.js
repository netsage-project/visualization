function LoadData(queryDate,queryText,avgOver,queryType,queryMeasure,queryValue){
	...
	function drawNetworkSummary(query){
		...
		let netSummary = d3.select("#AppRegion"+counter)
	      ...
	      //We need to know if we are in dashboard or not for the position of the infobutton
	      if(window.location.pathname==="/dashboard.html" || window.location.pathname==="/netsage/dashboard.html"){
		      d3.select("#topAppRegion"+counter).append("img")
				.attrs({
					"class":"askNetSageInfoDashBoard noShowInfo",
					"src":"question-mark.png"
				})
			  	.on("click",handleClickDashboard);
		  }else{
		  	d3.select("#topAppRegion"+counter).append("img")
				.attrs({
					"class":"askNetSageInfo noShowInfo",
					"src":"question-mark.png"
				})
	      ...
	      function handleClick(){
			...
			if(this.classList[1]==="showInfo"){
				d3.select(this)
			  	  .attr("class","askNetSageInfo noShowInfo")
			}else{
				d3.select(this)
			  	  .attr("class","askNetSageInfo showInfo")
			}
			if(this.classList[1]==="showInfo")
			{
		...
		function handleClickDashboard(){
			...
			if(this.classList[1]==="showInfo"){
				d3.select(this)
			  	  .attr("class","askNetSageInfoDashBoard noShowInfo")
			}else{
				d3.select(this)
			  	  .attr("class","askNetSageInfoDashBoard showInfo")
			}
			if(this.classList[1]==="showInfo")
			{
		...
		function createInfoText(location){
			...
			//Map
			text1 = "<p>The map shows the state of the links and nodes - the size of the links represent the relative size of the connection. Links are colored in a blue scale while nodes are colored using an orange scale, gray links and white nodes have no data for the chosen time frame.</p> ";
			//Table
			...
			text21 = "<p> Distribution of bandwidth measurements for a specific link- The width of the bar represents the maximum measurement for the chosen period across all of the links shown. This allows for a proper comparison of bandwidth distribution across all the links. The top bar represents bandwidth from Z to A and the bottom bar represents bandwidth in the opposite direction. Each vertical tick mark represents a single measurement. The denser the collection of marks, the more transmissions were found concentrated at specific bandwidth levels. </p>";
			text22 = " <p> Traffic Volume shows the bandwidth used in the measurement period. The top graph shows bandwidth from Z to A, and the bottom graph shows the reverse. </p>";
			text23 = " <p> These two columns show individual views of bandwidth distribution  for the links and nodes, as well as their maximum and average values. Note that the axes on the histograms do not share the same ranges. Rather they use ranges to allow for a more detailed view of bandwidth distribution.</p>";
			text24 = "<p> Amount of data transferred per network element relative to the total  data transferred by all the network elements.</p>";
			...
			//Dashboard Info
			text0 = "<p> The NetSage dashboard updates every 5 minutes showing the last 24 hours of information about IRNC Network. Click on Ask NetSage on the left to perform a custom query.</p>";
			...
			//LineChart
			text3 = "<p> These charts combine the bandwidth data for all the network elements in the chosen period. </p>"
